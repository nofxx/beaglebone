  ...
  module AIN
    class << self
      ...
      def wait_for_threshold(pin, mv_lower, mv_upper, mv_reset = 10, interval = 0.01, mv_last = nil, state_last = nil)
        Beaglebone.check_valid_pin(pin, :analog)
        raise ArgumentError, "mv_upper needs to be between 0 and 1800: #{pin} (#{mv_upper})" unless (0..1800).cover?(mv_upper)
        raise ArgumentError, "mv_lower needs to be between 0 and 1800: #{pin} (#{mv_lower})" unless (0..1800).cover?(mv_lower)
        raise ArgumentError, "mv_lower needs to be <= mv_upper: #{pin} (#{mv_lower}:#{mv_upper})" unless mv_lower <= mv_upper
        raise ArgumentError, "mv_reset needs to be between 0 and 1800: #{pin} (#{mv_reset})" unless (0..1800).cover?(mv_reset)
        ...
        unless state_last
          state_last = if mv_last >= mv_upper
                         :HIGH
        ...
        loop do
          ...
          if state_last == :LOW
            ...
            state = if mv >= mv_upper && mv >= mv_lower + mv_reset
                      :HIGH
            ...
            state = if mv <= mv_lower && mv <= mv_upper - mv_reset
                      :LOW
      ...
      def wait_for_change(pin, mv_change, interval, mv_last = nil)
        Beaglebone.check_valid_pin(pin, :analog)
        raise ArgumentError, "mv_change needs to be between 0 and 1800: #{pin} (#{mv_change})" unless (0..1800).cover?(mv_change)
